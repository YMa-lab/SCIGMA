...
class Model(Module):
    def __init__(self, dim_ins = {}, dim_hid = {}, dim_outs = {}, dropout=0.1, layer='GAT', device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
        ...
        if self.layer == 'GAT':
            ...
            for key in dim_ins:
                enc = GAT(2, [8,2], [dim_ins[key], dim_hid[key], dim_outs[key]]).to(self.device)
                dec = GAT(2, [2,8], [dim_outs[key], dim_hid[key], dim_ins[key]]).to(self.device)
        ...
        self.mod_temp = nn.Sequential(
                          nn.Linear(dim_outs[key], dim_outs[key]//2),
                          nn.LeakyReLU(),
                          nn.Linear(dim_outs[key]//2,1),
                        )


        self.graph_cross = Attention(dim_outs[key], dim_outs[key])
        self.graph_uncross = nn.Linear(dim_outs[key], dim_outs[key]*len(dim_outs))
    ...
    def forward(self, features, edges_sp):
        if self.layer == 'GAT':
            edges = {}
            for key in edges_sp:
                edges[key] = edges_sp[key].indices().to(self.device)
                
            latents = {}
            for key in features:
                emb_latents = self.encoders[key]([features[key], edges[key]])[0]
                latents[key] = emb_latents
           
            
            # between-modality attention aggregation
            emb_latent_combined, attention = self.graph_cross(latents)
            
            
            emb_latent_recon = self.graph_uncross(emb_latent_combined)
            latent_recons_list = torch.split(emb_latent_recon,emb_latent_combined.shape[1],dim=1)
            latent_recons = {}
            for i,key in enumerate(features):
                latent_recons[key] = latent_recons_list[i]
            
            recons = {}
            for key in features:
                re = self.decoders[key]([latent_recons[key], edges[key]])[0] 
                recons[key] = re
...
class Attention(Module):
    ...
    def forward(self, embs):
        emb = []
        for key in embs:
            emb.append(torch.unsqueeze(torch.squeeze(embs[key]), dim=1))
